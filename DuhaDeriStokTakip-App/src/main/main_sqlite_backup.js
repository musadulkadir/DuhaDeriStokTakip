...
async function createTables() {
  try {
    ...
    for (const color of defaultColors) {
      await client.query('INSERT INTO colors (name, hex_code) VALUES ($1, $2) ON CONFLICT (name) DO NOTHING', [color.name, color.hex]);
    }
    ...
    try {
      await client.query(`
        ...
        ADD COLUMN IF NOT EXISTS type VARCHAR(20) DEFAULT 'customer'
      `);
...
ipcMain.handle('customers:get-all', async (_, page = 1, limit = 50) => {
  ...
  try {
    ...
    const result = await client.query('SELECT * FROM customers ORDER BY created_at DESC LIMIT $1 OFFSET $2', [limit, offset]);
...
// YENİ EKLENECEK HANDLER:
ipcMain.handle('sales:getById', async (event, saleId) => {

  // Senin 'sales:get-all' sorgunun aynısı,
  // Sadece sonuna 'WHERE s.id = ?' eklendi
  const query = `
    SELECT 
      s.*,
      c.name as customer_name,
  ...
  return new Promise((resolve, reject) => {
    db.all(query, [saleId], (err, saleDetailsRows) => {
      if (err) {
      ...
      try {
        ...
        // KRİTİK ADIM: Veri Formatlama
        // Sorgu, her 'item' için bir satır döndürür.
        // Biz bunu (SalesManagement'taki gibi) tek bir Sale objesine dönüştürmeliyiz.

        const firstRow = saleDetailsRows[0];

        // firstRow'un undefined olmadığından emin olalım
        if (!firstRow) {
        ...
        const formattedSale = {
          ...
          total: firstRow?.total_amount || 0, // Ana 'sales' tablosundaki genel toplam
          date: firstRow?.sale_date || null,

          // Tüm satırları (items) bir diziye dönüştür
          items: saleDetailsRows.map(row => ({
            ...
            total: row.total_price // Bu, 'sale_items' tablosundaki kalem toplamıdır
          })).filter(item => item.productId) // 'LEFT JOIN'den gelebilecek boş (satış var ama kalem yoksa) kayıtları filtrele
        };

        resolve({ success: true, data: formattedSale });

      } catch (error) {
        console.error('Error processing sale data:', error);
        resolve({ success: false, error: error.message || 'Veri işleme hatası' });
      }
    });
  });
});
// YENİ EKLENECEK ALIM HANDLER'I:
ipcMain.handle('purchases:getById', async (event, purchaseId) => {

  // 'sales:getById' sorgusunun 'purchases' için uyarlanmış hali
  const query = `
    SELECT 
      p.*,
  ...
  try {
    const purchaseDetailRows = await db.all(query, [purchaseId]); // 'db.all' senin DB sürücüne göre değişebilir

    if (!purchaseDetailRows || purchaseDetailRows.length === 0) {
      return { success: false, error: 'Alım kaydı bulunamadı' };
    }

    // Veriyi formatla (sales:getById ile aynı mantık)
    const firstRow = purchaseDetailRows[0];
    ...
    const formattedPurchase = {
      ...
      date: firstRow.purchase_date, // 'purchases' tablosundaki tarih kolonunun adı

      items: purchaseDetailRows.map(row => ({
        productId: row.product_id,
        productName: `${row.category || 'Bilinmiyor'} - ${row.color || 'Bilinmiyor'}`,
        quantityPieces: row.quantity_pieces,
        quantityDesi: row.quantity_desi,
